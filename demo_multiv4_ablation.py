...
# Full PulseCache（含标定指纹）
full_qasm_cache = Memory("pc_full_qasm", verbose=0)
@full_qasm_cache.cache
def _full_compile_from_qasm(qasm_str: str):
    """把 QASM 反序列化 → transpile →（可选）schedule"""
    qc  = QuantumCircuit.from_qasm_str(qasm_str)
    tqc = transpile(qc, backend, optimization_level=3)
    if HAS_PULSE:
        schedule(tqc, backend=backend)
    return tqc
def full_qasm_flow(qc: QuantumCircuit):
    qasm_key = qc.qasm()                       # 也可用 sha256(qasm) 做短 key
    t0 = time.perf_counter()
    _full_compile_from_qasm(qasm_key)          # <<<<<<<<<<< 传字符串
...
def run_all(runs: int):
    res = {p: {c: [] for c in CIRCS} for p in ["Baseline", "Full", "Full_qasm"] + POLICIES}
...
def bar_variant(circ: str, res: Dict[str, Dict[str, List[float]]], var: str):
    ...
    ensure_dir("figs"); plt.savefig(f"figs/v4a_{circ}_{var}.png", dpi=300)
    plt.close()

def multi_variant_bar(circ: str, res: Dict[str, Dict[str, List[float]]]):
    labels = ["Baseline", "Full", "Full_qasm"] + POLICIES
    colors = ["tab:gray", "tab:blue", "tab:cyan", "tab:orange", "tab:green", "tab:red"]
    ...
    ensure_dir("figs"); plt.savefig(f"figs/v4a_{circ}_AllVariants.png", dpi=300)
    plt.close()
