...
def flow_full(qc: QuantumCircuit):
...
def _hash_compile(qc):
    return _full_compile_from_qasm(qc.qasm())
# @hash_cache.cache
# def _hash_compile(qc):
...
# def hot_circ():
#     name = np.random.choice(list(CIRCS))
#     depth = 16
#     return CIRCS[name](num_qubits(backend), depth)

# def task(policy):
#     return policy, FLOW[policy](hot_circ())

def run_concurrent(rate=20, total=5000): # 5000
...
def plot_cdf(lat):
    ...
    plt.savefig('figs/v4hqasm_cdf_concurrent.png', dpi=300)
