...
COMPONENTS  = ["transpile", "schedule", "lookup", "calibration"]
COL_STAGE   = dict(lookup="#1E90FF", transpile="#E9967A",
                   schedule="#FFD39B",  calibration="#8FBC8F")
...
time_meta: dict[str, tuple[float,float]] = {}

def _full_compile_helper(qasm_str: str):
    t0 = time.perf_counter()
    qc_raw = QuantumCircuit.from_qasm_str(qasm_str)
    tqc    = transpile(qc_raw, backend, optimization_level=3)
    ...
    if HAS_PULSE:
        ...
        time.sleep(0.3)
    t_sc  = time.perf_counter() - t1

    # 仅在编译阶段（第一次）写 meta；命中时不会重新覆盖
    time_meta.setdefault(qasm_str, (t_tr, t_sc))
    return tqc

@full_cache.cache
def _full_compile(qasm_str: str):
    return _full_compile_helper(qasm_str)                        # ← 缓存里还是 QuantumCircuit
...
def _tr_compile(qasm_str: str):
    ...
    t_tr = time.perf_counter() - t0
    time_meta.setdefault(qasm_str, (t_tr, 0.0))
    return tqc
...
def flow_full(qc):
    ...
    if _full_compile.check_call_in_cache(qasm_str):   # —— 已缓存
        # 只计 lookup 时间
        with timer(s, "lookup"):
            _ = _full_compile(qasm_str)               # 反序列化
    else:                                             # —— 第一次编译
        _ = _full_compile(qasm_str)                   # 真正编译 + 写 cache
        tr, sc = time_meta[qasm_str]                  # 从 meta 读两段耗时
        s["transpile"] = tr
        s["schedule"]  = sc
    with timer(s, "calibration"):
        time.sleep(0.05)
    return s
...
def flow_hash(qc):
    s = dict.fromkeys(COMPONENTS, 0.0)
    qasm_str = qc.qasm()
    if _hash_compile.check_call_in_cache(qasm_str):  # —— 已缓存
        # 只计 lookup 时间
        with timer(s, "lookup"):
            _ = _hash_compile(qasm_str)  # 反序列化
    else:  # —— 第一次编译
        _ = _hash_compile(qasm_str)  # 真正编译 + 写 cache
        tr, sc = time_meta[qasm_str]  # 从 meta 读两段耗时
        s["transpile"] = tr
        s["schedule"] = sc
    return s
...
def flow_transpile(qc):
    s = dict.fromkeys(COMPONENTS, 0.0)
    qasm_str = qc.qasm()
    if _tr_compile.check_call_in_cache(qasm_str):  # —— 已缓存
        # 只计 lookup 时间
        with timer(s, "lookup"):
            tqc = _tr_compile(qasm_str)  # 反序列化
    else:  # —— 第一次编译
        tqc = _tr_compile(qasm_str)  # 真正编译 + 写 cache
        tr, sc = time_meta[qasm_str]  # 从 meta 读两段耗时
        s["transpile"] = tr
    if HAS_PULSE:
        with timer(s,"schedule"): schedule(tqc, backend=backend)
...
def bench_breakdown(pol:str, gen:Callable, depth:int, runs:int):
    ...
    return {c: np.sum([t[c] for t in trials]) for c in COMPONENTS}
...
def plot_circuit_breakdown(circ:str, data):
    ...
    for i, pol in enumerate(POLICIES):
        ...
        for comp in COMPONENTS:
            # width = data[pol][circ][comp][depth_idx]
            width = np.sum(data[pol][circ][comp][ind] for ind in range(len(DEPTHS)))
            ax.barh(i, width, left=bottom, color=COL_STAGE[comp], height=0.4,
                    edgecolor="black", linewidth=0.6,
                    label=comp if (i==0) else None, zorder=3)
            bottom += width
    ax.grid(axis="x", linestyle="--", linewidth=0.6, alpha=0.5, zorder=0)
    ax.set_yticks(y_pos, POLICIES, fontweight="bold")
    ...
    ax.legend(loc="upper right", frameon=False)
    plt.tight_layout(); ensure_dir("figs")
    plt.savefig(f"figs/v5abd3_allDepth_{circ}.png", dpi=600)
    plt.close()
...
if __name__ == "__main__":
    ...
    save_path = Path(f"savedRes/results_demo5abd3.pkl")
    ...
    for circ in CIRCS:
        plot_circuit_breakdown(circ, results)

    print("✅ finished! Stacked bar chart saved to figs/v5abd3_allDepth_{circ_name}.")
