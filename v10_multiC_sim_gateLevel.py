...
FIGDIR  = pathlib.Path("./figs/v10"); FIGDIR.mkdir(exist_ok=True)
CACHE_P = pathlib.Path("./figs/v10/v10_sim_transpile_cache.pkl")   # {key: qc_transpiled}
code_tag = "v10"
...
# ================= batch runners =================
def run_circuits(runs: int, backend, shots: int):
    results = {}
    for name, fn in CIRCUITS.items():
        ...
        for r in range(runs):
            ...
            (FIGDIR / f"{code_tag}_sim_{name}_baseline_{r}.json").write_text(json.dumps({"laps": laps, "meta": meta}, indent=2))
        base_sum = {k: round(sum(rr.get(k, 0.0) for rr in base_runs), 6) for k in base_runs[0]}
        (FIGDIR / f"{code_tag}_sim_{name}_baseline_sum.json").write_text(json.dumps(base_sum, indent=2))
        ...
        for r in range(runs):
            ...
            pc_runs.append(laps)
            (FIGDIR / f"{code_tag}_sim_{name}_tcache_{r}.json").write_text(json.dumps({"laps": laps, "meta": meta}, indent=2))
        cache_sum = {k: round(sum(rr.get(k, 0.0) for rr in pc_runs), 6) for k in pc_runs[0]}
        (FIGDIR / f"{code_tag}_sim_{name}_tcache_sum.json").write_text(json.dumps(cache_sum, indent=2))

        results[name] = (base_sum, cache_sum)

    plot_all(results, FIGDIR / f"{code_tag}_sim_compare_multi.png")
...
if __name__ == "__main__":
    ...
    if args.circuit:
        ...
        run_circuits(args.runs, backend, args.shots)
