...
def plot_timeline(events: List[Dict[str, Any]], out_png: pathlib.Path):
    ...
    xs_off, ys_off, labs_off = [], [], []        # <-- 新增 OFFLOAD
    ...
    for e in events:
        x = e["ts"] - t0
        if e["type"] == "prewarm":
            xs_pre.append(x); ys_pre.append(1.0)
            # labs_pre.append(label_for_key(e["key"]))
            labs_pre.append(event_label_with_seq(e))
        elif e["type"] == "OFFLOAD":
            xs_off.append(x); ys_off.append(0.5)
            # labs_off.append(label_for_key(e["key"]))  # 中间轨
            labs_off.append(event_label_with_seq(e))
        elif e["type"] == "run":
    ...
    pre = ax.scatter(xs_pre, ys_pre, marker="^", s=70, label="prewarm (compiled)")
    off = ax.scatter(xs_off, ys_off, marker="v", s=70, label="offload (evicted)", color="tab:purple")  # 新增
    rh  = ax.scatter(xs_run_hit,  ys_run_hit,  marker="o", s=70, label="run cacheIdle — HIT",  color="tab:green")
    rm  = ax.scatter(xs_run_miss, ys_run_miss, marker="o", s=70, label="run cacheIdle — MISS", color="tab:red")

    def annotate(xs, ys, labs, dy):
        for xi, yi, lb in zip(xs, ys, labs):
            ax.annotate(lb, (xi, yi+dy), xytext=(0, 6 if dy>=0 else -8),
                        textcoords="offset points", rotation=-90,
                        ha="left", va="bottom" if dy>=0 else "top", fontsize=6, alpha=0.9)

    annotate(xs_pre, ys_pre, labs_pre, dy=+0.03)
    annotate(xs_off, ys_off, labs_off, dy=+0.03)       # 新增
    annotate(xs_run_hit, ys_run_hit, labs_run_hit, dy=-0.03)
    annotate(xs_run_miss, ys_run_miss, labs_run_miss, dy=-0.03)

    ax.set_yticks([0.0, 0.5, 1.0])
    ax.set_yticklabels(["Run (cacheIdle)", "Offload", "Prewarm"])
    ...
    ax.legend(
        ...
        bbox_to_anchor=(0.5, 1.22),  # y > 1 把它顶到图框外；可按需求微调
        ncol=4,  # 一行显示多列条目
        frameon=False
    )
    # plt.tight_layout()
