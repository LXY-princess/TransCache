...
def plot_timeline(events: List[Dict[str, Any]], out_png: pathlib.Path):
    """
    三行子图 (sharex=True)：
      Row-1: 主时间轴（prewarm/offload/run(HIT/MISS)），标记内部写 c1/c2/…
      Row-2: Cache Size（仅在 cache 变化事件处采样；阶梯线 + 散点）
      Row-3: Cache Contents（仅在变化时刻，竖排写当前 cache 的 c1/c2/… 列表）
    右侧：短码说明（c1 → 原始电路名_q_d）。
    """
    if not events:
        print("No timeline events to plot.")
        return

    # 事件按时间排序，并构建 key→短码 的映射
    events_sorted = sorted(events, key=lambda e: e["ts"])
    t0 = events_sorted[0]["ts"]
    t_end = events_sorted[-1]["ts"]
    key_to_code, ordered_pairs = build_codebook(events_sorted)

    # ---------------- Row-1 数据：散点 + 短码 ----------------
    xs_pre, ys_pre, codes_pre = [], [], []
    xs_off, ys_off, codes_off = [], [], []
    xs_run_hit, ys_run_hit, codes_run_hit = [], [], []
    xs_run_miss, ys_run_miss, codes_run_miss = [], [], []

    for e in events_sorted:
        x = e["ts"] - t0
        k = e.get("key")
        code = key_to_code.get(k, "c?")
        if e["type"] == "prewarm":
            xs_pre.append(x); ys_pre.append(1.0); codes_pre.append(code)
        elif e["type"] == "OFFLOAD":
            xs_off.append(x); ys_off.append(0.5); codes_off.append(code)
        elif e["type"] == "run":
            if e.get("hit", False):
                xs_run_hit.append(x);  ys_run_hit.append(0.0); codes_run_hit.append(code)
            else:
                xs_run_miss.append(x); ys_run_miss.append(0.0); codes_run_miss.append(code)

    # ---------------- Row-2/3 数据：根据事件重建 cache 轨迹 ----------------
    cache_state: set[str] = set()
    change_times: List[float] = []
    cache_sizes: List[int] = []
    cache_snap_labels: List[str] = []  # 只写短码

    def _codes_for_state(st: set) -> str:
        # 按 c 序号升序输出（c1,c2,…）
        codes = [key_to_code[k] for k in st if k in key_to_code]
        codes.sort(key=lambda s: int(s[1:]) if s[1:].isdigit() else 10**9)
        return "\n".join(codes) if codes else "(empty)"
    ...
    for e in events_sorted:
        ...
        if e["type"] == "prewarm":
            if k not in cache_state: cache_state.add(k); mutated = True
        elif e["type"] == "OFFLOAD":
            if k in cache_state: cache_state.remove(k); mutated = True
        ...
        if mutated:
            ...
            cache_snap_labels.append(_codes_for_state(cache_state))

    if not change_times:
        change_times, cache_sizes, cache_snap_labels = [0.0], [0], ["(no cache changes)"]

    # ---------------- 作图：三行子图 + 右侧短码说明 ----------------
    fig, (ax0, ax1, ax2) = plt.subplots(
        3, 1, figsize=(13.8, 7.8), sharex=True,
        gridspec_kw={"height_ratios": [3.0, 1.2, 1.8]}
    )

    # Row-1：主时间轴（先画散点，再把 cX 写进标记内部）
    pre = ax0.scatter(xs_pre, ys_pre, marker="^", s=80, label="prewarm (compiled)")
    off = ax0.scatter(xs_off, ys_off, marker="v", s=80, label="offload (evicted)", color="tab:purple")
    rh  = ax0.scatter(xs_run_hit,  ys_run_hit,  marker="o", s=80, label="run cacheIdle — HIT",  color="tab:green")
    rm  = ax0.scatter(xs_run_miss, ys_run_miss, marker="o", s=80, label="run cacheIdle — MISS", color="tab:red")

    _draw_codes_inside(ax0, xs_pre, ys_pre, codes_pre, fontsize=7)
    _draw_codes_inside(ax0, xs_off, ys_off, codes_off, fontsize=7)
    _draw_codes_inside(ax0, xs_run_hit, ys_run_hit, codes_run_hit, fontsize=7)
    _draw_codes_inside(ax0, xs_run_miss, ys_run_miss, codes_run_miss, fontsize=7)
    ...
    ax0.legend(loc="upper center", bbox_to_anchor=(0.45, 1.30), ncol=4, frameon=False)

    # Row-2：Cache Size（阶梯线 + 散点）
    ax1.step(change_times, cache_sizes, where="post", linewidth=1.8, alpha=0.9)
    ax1.plot(change_times, cache_sizes, "o", ms=4)
    ax1.set_ylabel("Cache size")
    ax1.grid(axis="x", linestyle="--", alpha=0.4)
    ax1.set_ylim(bottom=0)

    # Row-3：Cache Contents（竖排短码）
    ax2.set_ylim(0.0, 1.0)
    ax2.set_yticks([])
    ax2.set_ylabel("Cache contents", labelpad=6)
    for xi, txt in zip(change_times, cache_snap_labels):
        ax2.axvline(x=xi, color="#dddddd", linewidth=0.8, zorder=0)
        ax2.text(xi, 0.02, txt, rotation=0, ha="left", va="bottom", fontsize=7)
    ...
    # ------------ 右侧：短码→原始电路名 的“legend” --------------
    # 在图的右边加一个纯文本轴，逐行显示 cX = 原始电路名_q_d
    axR = fig.add_axes([0.84, 0.12, 0.14, 0.78])   # [left, bottom, width, height] in figure coords
    axR.axis("off")
    axR.set_title("Codebook", loc="left", fontsize=10, pad=4)
    # 逐行写入（保持与分配顺序一致）
    y = 0.98
    for k, code in ordered_pairs:
        axR.text(0.0, y, f"{code} = {label_for_key(k)}", transform=axR.transAxes,
                 ha="left", va="top", fontsize=8)
        y -= 0.06  # 行距可按需要调整

    # 预留右侧空间给 codebook + 上方空间给事件 legend
    plt.tight_layout(rect=[0.02, 0.0, 0.82, 0.88])
...
def build_codebook(events_sorted):
    """
    为所有出现过的 key 分配短码 c1,c2,…（按“首次出现时间”稳定排序）。
    返回 (key_to_code: dict, ordered_pairs: List[(key, code)])。
    """
    first_seen = {}
    for e in events_sorted:
        k = e.get("key")
        if k is not None and k not in first_seen:
            first_seen[k] = e["ts"]
    ordered = sorted(first_seen.items(), key=lambda kv: kv[1])
    key_to_code = {k: f"c{i+1}" for i, (k, _) in enumerate(ordered)}
    return key_to_code, [(k, key_to_code[k]) for k, _ in ordered]

def _draw_codes_inside(ax, xs, ys, codes, fontsize=7):
    """
    在散点标记内部写入短码（白底黑描边以保证可读性）。
    """
    for xi, yi, code in zip(xs, ys, codes):
        txt = ax.text(xi, yi, code, ha="center", va="center",
                      fontsize=fontsize, color="white", fontweight="bold")
        txt.set_path_effects([patheffects.withStroke(linewidth=1.6, foreground="black")])

# ============ CLI ============
if __name__ == "__main__":
    ap = argparse.ArgumentParser()
    ap.add_argument("--shots", type=int, default=512)
    ap.add_argument("--runs", type=int, default=2, help="每个电路重复 [prewarm->run] 的轮数")
