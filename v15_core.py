...
def build_workload_poisson_superposition(meta, workload_len: int,
    ...
    # rps rad/s speed, total sim window length is H seconds.
    H = workload_len / max(1e-9, rps)

    # randomly make m circuits as hot circuits
    # Multiple their weights with hot_boost
    m = len(meta)
    weights = np.ones(m, dtype=float)
    hot_k = max(1, int(round(hot_fraction * m)))
    ...
    # 归一化后得到每类的到达强度lambdas（单位：req/s）
    w_sum = float(weights.sum())
    lambdas = (weights / w_sum) * rps

    # 按每类的 λ 生成各自的泊松过程事件（指数间隔）
    events: List[Tuple[float, int]] = []
