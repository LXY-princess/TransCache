...
def main_run(args):
    ...
    for si, N in enumerate(sizes):
        ...
        for r in range(1, R+1):
            ...
            # 1) 欠/过离散的更新过程（Renewal）
            wl_gamm, info_gamm = build_workload_renewal_exact(meta, N, args.rps, renewal_kind="gamma", shape=3.0,
                                                              class_mode="weighted", rng_seed=rng_seed)
            visualize_workload_common(wl_gamm, info_gamm, meta, out_dir="figs/v18/plots_renewal_gamma_k3")
            workload, info = wl_gamm, info_gamm
