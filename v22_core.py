...
# --------- compile/run helpers ----------
def compile_with_idle_cache_ibm(qc_raw: QuantumCircuit, bk_name: str,
                            cache: Dict[str, QuantumCircuit],
                            backend: Any) -> Tuple[QuantumCircuit, str, bool, float]:
...
def run_once_with_cache_ibm(qc_func: Callable[[], QuantumCircuit],
                        cache: Dict[str, QuantumCircuit],
                        shots: int = 256,
    ...
    if include_exec:
        ...
        log_ts = met.get("timestamps", {}) if isinstance(met, dict) else {}
        # timestamps keys may vary slightly among releases; guard accesses:
        t_created = parse_iso(log_ts.get("created")) if log_ts.get("created") else None
        t_running = parse_iso(log_ts.get("running")) if log_ts.get("running") else None
        t_finished = parse_iso(log_ts.get("finished")) if log_ts.get("finished") else None
    else:
        exec_sec = 0.0  # ← 不执行，置零
        t_created = 0.0
        t_running = 0.0
...
def run_once_nocache_ibm(qc_func: Callable[[], QuantumCircuit], shots: int = 256, include_exec: bool = True,
    ...
    if include_exec:
        ...
        svc = QiskitRuntimeService()
        job_id = "d3skijk60rgc73aacddg" #1."d3sk5hejbaoc73cne6b0" 2."d3sk5mujbaoc73cne6gg" 3."d3sk5ts60rgc73aac1b0", 4."d3sk64c60rgc73aac1i0", 5."d3skijk60rgc73aacddg"
        job = svc.job(job_id)
