...
from v22_core import (PoissonPredictor, label_of,
                      seed_recent_calls_for_predictor, run_once_with_cache_ibm)
from v22_prewarm import (prewarm_simple_topk_ema_ibm)
...
def run_strategy(workload, makers_all,
    ...
    for idx, it in enumerate(workload):
        ...
        if (idx % prewarm_every) == 0:
            inserted = prewarm_simple_topk_ema_ibm(
                predictor, makers_all, now_t=t,
                lookahead_sec=lookahead_sec, prob_th=prob_th,
                cache=cache,
                est_compile_ema=est_compile_ema,
                default_compile_est=default_compile_est,
                max_compile=max_compile,
                last_used_t=last_used_t,  # 可选：让函数里顺便刷新TTL时钟
                backend=backend,
                # on_compiled=lambda lab, key, cost: ... # 可选：回调里做日志/指标
                # return_details=True                    # 可选：想拿到每个key的compile_sec
            )
